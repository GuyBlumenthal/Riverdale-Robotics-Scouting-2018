...
public class MatchDetail extends JFrame {
	...
	public MatchDetail() {
		...
		txtMatchNumber.setBounds(423, 48, 62, 20);
		...
		DefaultTableModel mdlSwitch = new DefaultTableModel(
				new Object[][] {
					{new Boolean(false), new Boolean(false), "0:00"},
				},
				new String[] {
	...
	public Team setTableData(Team team, JTable tblTable){
		...
		for (int i = 1; i < tblTable.getRowCount() - 1; i++) {	
			...
			if((Boolean)tblTable.getModel().getValueAt(i, 0)){
				...
				if(tblTable.getName().equals("tblSwitch")){
					...
					if((Boolean)tblTable.getModel().getValueAt(i, 1))	{isDefense++;}	else	{isOffense++;}
		...
		team.numCubesOnSwitchAuto.add(nCubesInAuto); 	/*TODO:fix*/
		team.numCubesOnAllianceSwitch.add(isDefense); 	/*TODO:fix*/
		team.numCubesOnOpponentsSwitch.add(isOffense); 	/*TODO:fix*/
		
		int nAuto = 0, nTeleop = 0;
		
		switch(tblTable.getName()){
			case "tblSwitch": 	nAuto = 0; 	nTeleop = 1; 	break;
			case "tblScale":	nAuto = 2;	nTeleop = 3;	break;
			case "tblVault":	nAuto = 4; 	nTeleop = 4;	break;
		}
